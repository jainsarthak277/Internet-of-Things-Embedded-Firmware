...
void initiate_time_reset(void)
{
	...
	setDateAndTime(SET_YEAR,SET_MONTH,SET_DATE,SET_DAY,SET_HOUR,SET_MINUTE,SET_SEC,SET_MSEC);
}
...
void fire_response(void)
{
	fire_detected++;
	LCD_write("FIRE...FIRE", 6);
	gecko_cmd_hardware_set_soft_timer(32768*15, TIMER_ID_FIRE_RESET, 1);
	gecko_cmd_hardware_set_soft_timer(32768/2, TIMER_ID_FIRE_LED0, 0);
}
...
static errorcode_t onoff_update_and_publish(uint16_t element_index)
{
  ...
  e = onoff_update(element_index);
	...
	req.on_off = lightbulb_state.fire_current ? MESH_GENERIC_ON_OFF_STATE_ON : MESH_GENERIC_ON_OFF_STATE_OFF;
...
void set_device_name(bd_addr *pAddr)
{
  ...
  sprintf(name, "%x:%x", pAddr->addr[1], pAddr->addr[0]);
...
void enable_button_interrupts(void)
{
  ...
  GPIO_ExtIntConfig(BSP_BUTTON1_PORT, BSP_BUTTON1_PIN, BSP_BUTTON1_PIN, true, false, true);
...
void friend_node_init(void)
{
    ...
    else printf("Friend initialization failed 0x%x\n\r", resnew);
    LCD_write("", 6);
...
static void handle_gecko_event(uint32_t evt_id, struct gecko_cmd_packet *evt)
{
  switch (evt_id) {
    ...
    case gecko_evt_system_boot_id:
        // check pushbutton state at startup. If both PB0 and PB1 are held down then do time reset
        ...
        if ((GPIO_PinInGet(BSP_BUTTON0_PORT, BSP_BUTTON0_PIN) == 0 || GPIO_PinInGet(BSP_BUTTON1_PORT, BSP_BUTTON1_PIN) == 0) && time_rst_done == 0)
        {
...
//      gecko_cmd_mesh_node_init_oob(0x00, 0x08, 0x08, 0x02, 0x00, 0x00, 0x00);
//      gecko_cmd_mesh_node_init_oob(0x00, 0x03, 0x08, 0x04, 0x04, 0x04, 0x01);0234440(Vipul code)
...
//        	if((seconds_2_send > SEC_THRESH_1 && seconds_2_send < SEC_THRESH_1 + 5) || ((seconds_2_send > SEC_THRESH_2 && seconds_2_send < SEC_THRESH_2 + 5)))
        	req.on_off = ((seconds_2_send > 0) && (seconds_2_send < 5)) || ((seconds_2_send > 30) && (seconds_2_send < 35))? MESH_GENERIC_ON_OFF_STATE_ON : MESH_GENERIC_ON_OFF_STATE_OFF;
        	if((seconds_2_send > 0 && seconds_2_send <  5) || ((seconds_2_send > 15 && seconds_2_send < 20)) || ((seconds_2_send > 30 && seconds_2_send < 35)) || ((seconds_2_send > 45 && seconds_2_send < 50)))
        	{
...
//        	gecko_cmd_hardware_set_soft_timer(32768/2, TIMER_ID_CLIENT_GET, 1);
        	gecko_cmd_hardware_set_soft_timer(32768*5, TIMER_ID_DISPLAY_TIME, 1);
        	if (resp) {
        	 printf("gecko_cmd_mesh_generic_client_set state failed,code %x\r\n", resp);
        	} else {
        ...
        case TIMER_15_MIN:
        {
        	...
        	minutes_2_send = retrieve_minutes();
        		  ...
        		  req.kind = mesh_generic_request_on_off;
        		  req.on_off = (hours_2_send > HOUR_THRESH_1) && (hours_2_send < HOUR_THRESH_1 + 1) ? MESH_GENERIC_ON_OFF_STATE_ON : MESH_GENERIC_ON_OFF_STATE_OFF;
        		  if(((hours_2_send > HOUR_THRESH_1) && (hours_2_send < HOUR_THRESH_1 + 1) && (minutes_2_send > MIN_THRESH_1) && (minutes_2_send < MIN_THRESH_2)) || (((hours_2_send > HOUR_THRESH_2) && (hours_2_send < HOUR_THRESH_2 + 1) && (minutes_2_send > MIN_THRESH_1) && (minutes_2_send < MIN_THRESH_2))))
        		  {
        		  resp = gecko_cmd_mesh_generic_client_set(
        		    MESH_GENERIC_ON_OFF_CLIENT_MODEL_ID,
        		    _elem_index,
        ...
        case TIMER_ID_SENSOR:
        	...
        	{
        		fire_response();
        		sprintf(fire_LCD, "FIRE: eCO2=%d ppm", buffs[0]);
        		...
        		if (resp) {
      		    printf("Fiery gecko_cmd_mesh_generic_client_set state failed,code %x\r\n", resp);
      		  } else {
        ...
        {
        	uint8 resp;
        	resp = gecko_cmd_mesh_generic_client_get(
        			MESH_GENERIC_ON_OFF_CLIENT_MODEL_ID,
					_elem_index,
					server_lpn_address,
					appkey,
     ...
     case gecko_evt_mesh_node_initialized_id:
      ...
      else printf("Controller Initialization failed 0x%x\n\r", resnew);
      ...
      else printf("Beaconing failed 0x%x\n\r", resnew);
      }
...
//    	for(int i = 0; i < dOOB -> data.len; i++)
//    	{
//    		printf("%2.2x ", dOOB -> data.data[/*dOOB -> data.len - 1*/i]);
//    	}
//    	printf("\r\n");
//    	uint32_t oob_num = dOOB->data.data[dOOB->data.len -1] | (dOOB->data.data[dOOB->data.len -2]<<8) | (dOOB->data.data[dOOB->data.len -3]<<16);
//    	sprintf(passkeyOOB, "KEY:%2.2x", /*dOOB -> data.data[dOOB -> data.len - 1]*/oob_num);
...
//      gecko_cmd_hardware_set_soft_timer(TIMER_MS_2_TIMERTICK(50), TIMER_ID_RETRANS, 0);
    ...
    {
    	struct gecko_msg_mesh_generic_client_server_status_evt_t* lpn_pstatus = (struct gecko_msg_mesh_generic_client_server_status_evt_t*)&(evt->data.evt_mesh_generic_client_server_status);
    	maintain.para_slot_len = lpn_pstatus->parameters.len;
    	maintain.para_slot_data[maintain.para_slot_len - 1] = lpn_pstatus->parameters.data[lpn_pstatus->parameters.len - 1];

    	printf("Server status received\n\r");
    	...
    	if(evt->data.evt_mesh_generic_client_server_status.server_address == SERVER_LPN1_ADDRESS)
    	{
    		...
    		sprintf(pslot_status, "%s", maintain.para_slot_data[maintain.para_slot_len - 1] ? "Slot 1 full" : "Slot 1 empty");
    	...
    	if(evt->data.evt_mesh_generic_client_server_status.server_address == SERVER_LPN2_ADDRESS)
    	{
    		...
    		sprintf(pslot_status, "%s", maintain.para_slot_data[maintain.para_slot_len - 1] ? "Slot 2 full" : "Slot 2 empty");
